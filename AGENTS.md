...
1. **`python基礎語法/`** — Python 入門單元（unit01~unit06 + milestone-calculator）
2. **`AI 數據分析工作坊/`** — 數據分析工作坊，含 Pandas 講義與一套六階段的分析 Prompt 武器庫
...
```
├── python基礎語法/              # Python 入門單元
├── AI 數據分析工作坊/
│   ├── pandas數據分析工具教學/   # Pandas 講義 + 參考解答 (.ipynb)
│   └── 上課用prompt/            # 分析 Prompt 武器庫（六階段 00~05）
...
`AI 數據分析工作坊/上課用prompt/` 那六個階段是**給人用的教學模板**，本檔是**給你（agent）用的工作規範**。
